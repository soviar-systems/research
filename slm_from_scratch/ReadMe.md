...
- **Architectural Decoupling: Intelligence vs. State**: We reject the temptation to use the LLM's "intelligence" to solve "data" problems. Asking a model to "interpret a syllabus" to generate a roadmap is a design flaw; it invites the **Conciseness Trap**—a byproduct of RLHF where the model prioritizes visual cleanliness over informational completeness. To achieve 100% fidelity, we decouple the **Control Plane** (the Map) from the **Execution Plane** (the Mentor). The Architect owns the state; the Model executes the state. By providing a pre-formatted, exhaustive "Detailed Curriculum" section in the syllabus, we shift the LLM's role from *interpreting* the map (fragile) to *following* the map (robust).
    - **The RAG vs. Determinism Problem**: We discovered that relying on RAG (Retrieval-Augmented Generation) for the "Syllabus Synchronization Audit" is an architectural mismatch. RAG is **probabilistic**—it uses vector similarity to find "needles in a haystack." However, the audit requires **deterministic** retrieval of the "entire haystack" (the full syllabus).
    - **Query Drift**: High-density persona prompts can cause "Query Drift," where the model's internal intent shifts from operational retrieval (*"Find the syllabus file"*) to conceptual alignment (*"How do I act like a great mentor?"*). This results in the RAG returning fragmented, theoretically-relevant chunks rather than the structured document.
    - **Principle**: If a requirement is binary and absolute, do not ask the LLM to "reason" it—provide it as a fixed asset. Verbatim mirroring of the syllabus must be achieved through deterministic access (e.g., Static Injection or Tool-based reading) rather than probabilistic similarity search.
